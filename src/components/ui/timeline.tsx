    ...
    <div className="font-sans md:px-10" ref={containerRef}>
      <div ref={ref} className="relative max-w-7xl pb-20">
        {data.map((item, index) => (
          <div
            key={index}
            className="flex flex-col justify-start md:pt-20 md:gap-2"
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center self-start max-w-xs lg:max-w-sm md:w-full">
              <div className="h-6 absolute left-5 w-6 rounded-full bg-white dark:bg-black flex items-center justify-center">
                <div className="h-4 w-4 rounded-full bg-neutral-200 dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 p-2" />
              </div>
              <h3 className="hidden md:block text-xl md:pl-16 md:text-xl font-bold text-white">
                {item.title}
              </h3>
            </div>
